 ...
 *
 * Entrance is a smooth professional stagger (heading, then the cards' logos
 * settle in via `logo-in`, then the footer) with the same eased curve the
 * landing intro uses. All copy is short and derived from the bundled data -
 * no hardcoded run numbers.
 */
...
export function FamilySelect({
  ...
  return (
    <div className="flex h-screen flex-col items-center justify-center gap-10 overflow-y-auto bg-black px-6 py-10 text-center">
      <div style={{ animation: `card-in 1.1s ${EASE} 150ms backwards` }}>
      ...
      <div className="flex w-full max-w-2xl flex-col items-stretch justify-center gap-4 sm:flex-row">
        {families.map((family, i) => (
          <button
            ...
            style={{ animation: `logo-in 1s ${EASE} ${450 + i * 160}ms backwards` }}
          >
            <FamilyLogo
              family={family.id}
              markSize={52}
      ...
      <button
        ...
        style={{ animation: `card-in 1s ${EASE} 900ms backwards` }}
      >
        Back to landing
      </button>
    </div>
